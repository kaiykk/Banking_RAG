...
1. **显存不足**: 调整batch_size和gradient_accumulation_steps
2. **训练不稳定**: 降低learning_rate，增加warmup_steps
3. **过拟合**: 减少epochs，增加dropout

### RAG检索问题

1. **检索速度慢**: 优化FAISS索引类型，减少top_k
2. **检索不准确**: 微调嵌入模型，调整chunk_max_tokens
3. **重排序效果差**: 微调重排序模型，增加训练数据
